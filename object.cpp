...
//Object::Object(std::vector<glm::vec3> vertices, std::vector<glm::vec2> uvs, std::vector<glm::vec3> normals, std::string texturePath):m_vb(0), m_uvsb(0), m_texture(0), position(0,0,0), rotationAngles(0,0,0), m_reflectivity(0)
//{

//     m_vb = new VertexBuffer(vertices);
//     m_uvsb = new UVBuffer(uvs);
//     m_normalsb = new VertexBuffer(normals); // Initialize the normals buffer
//     m_texture = new Texture(texturePath);

//}

Object::Object(std::string objPath, std::string texturePath): m_vb(0), m_uvsb(0), m_normalsb(0), m_texture(0), position(0,0,0), rotationAngles(0,0,0), m_reflectivity(0), scaleFactor(1)
{
    std::vector<glm::vec3> vertices, normals;
    std::vector<glm::vec2> uvs;

    if (!loadOBJ(objPath.c_str(), vertices, uvs, normals)) {
        // Handle error loading the OBJ file
        std::cerr << "Error loading OBJ file: " << objPath << std::endl;
...
//    return m;
    ...
    m = glm::rotate(m, rotationAngles.x, glm::vec3(1, 0, 0));
    m = glm::rotate(m, rotationAngles.y, glm::vec3(0, 1, 0));
    m = glm::rotate(m, rotationAngles.z, glm::vec3(0, 0, 1));
    m = glm::scale(m, glm::vec3(scaleFactor, scaleFactor, scaleFactor)); // Add scaling here
    return m;
}
